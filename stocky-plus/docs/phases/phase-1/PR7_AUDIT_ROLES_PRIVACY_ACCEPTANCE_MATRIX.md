...
| PR7-CUST-119 | AO-06 table/manifest/extracted | extractor `--selftest` + extract | one-sided table or manifest hash edit; table-row removal; extra unmapped entry; version change; conflicting duplicate path in either order | fail closed; coordinated edit of every declaration not prevented | manifest self-hash cycle | extractor `extraction_table_manifest_mismatch`; `extraction_table_missing_path`; `extraction_table_unmapped_path`; `extraction_manifest_version`; `extraction_manifest_duplicate_path`; `extraction_authenticity_limit:coordinated_in_repo_edit_not_prevented` | Git pin + review | PR7 | executed-extractor-selftest |
| PR7-CUST-124 | F-CLAUDE-PR7SP-01 byte-identical duplicate path | extractor `--selftest` constructs an exact copy of a `files[]` row and inserts it first or last | duplicate path with identical bytes/hash in either order | `extraction_manifest_duplicate_path`; disagreement is not required | accepting a repeated path because hashes agree | extractor `byte_identical_duplicate_path_first`; `byte_identical_duplicate_path_second` | path identity | PR7 | executed-extractor-selftest |
...
| **F-CLAUDE-PR7AO-06** | P3 | §14 table vs manifest vs extracted bytes unchecked | **CORRECTED** (row-for-row; one-sided negatives; duplicate-path rejection; honest coordinated-edit limit) | PR7-CUST-119, 124; extractor `--selftest` |
| **F-CLAUDE-PR7SP-01** | P3 | Exact byte-identical duplicate `files[]` row accepted | **CORRECTED** (path identity; published byte-identical duplicate negative in either order) | PR7-CUST-124; extractor `--selftest` |
...
| Totals (this correction) | S **416/244/172** remains independently established. Clean-export of candidate `e7dd4f42…` **505/290/215** (G16 unique 93, NEG unique 15) on `/tmp/pr45-re-gate-pg16:5447`. Authoring `/tmp/pr45-re-pg16:5444` independently 505/290/215. This SP-01 working-tree re-run **505/290/215** on `/tmp/pr45-sp01-pg16:5448` (extractor `4b31d566…`; contract/driver unchanged). Do not relabel 505 as unique assertions |
