...
| PR7-CUST-078 | AO-02 computed commitment as source digest | `pr7-effect-v1` over frozen domain/shop/kind/value/effectId | ADMIN admit using computed hex as `sourceContentDigest` | admitted | treating source digest and effect commitment as unrelated | G16 `effect_host_admits_computed_commitment_as_source_digest` | identity transform | PR7 | executed-synthetic-pg16 |
...
| PR7-CUST-090 | AO-03 race capture then sighting | ADMIN capture then webhook sighting; follow-up new-identity capture | follow-up capture | denied; no double-bind | second identity binds same bytes | G16 `race_capture_then_sighting_does_not_double_bind` | source-content lock | PR7 | executed-synthetic-pg16 |
...
| Customer barrier | Canonical domain × `CUSTOMER_REST_ID` / `ORDER_LEGACY_ID` (`pr7-ctgt-v2`). Generation is evidence, not the lock. Lock before lookup. Not request-id-only. LIVE shop. Unrelated customers/shops progress. Delayed writes whose **BOUND** origin **is** the completed generation remain restore-denied; genuine unique LIVE successor with `originalAdmittedAt >= origin.installedAt` and a **new** digest allowed. Install lower bound is **supplementary**, not provenance. ADMIN requires original-admin capture. Unattributed inclusive overlap → visible `customer_target_attribution_ambiguous`. Unique LIVE is not historical origin. Guard derives origin from `WriterAdmissionOrigin`; `p_origin_persisted` removed. Effect host **computes** `pr7-effect-v1` from the frozen effect input and uses work identity only as a revalidated locator. GUC `stocky.trusted_work_id` is an untrusted locator. Capture unique `(canonicalDomain, commandId)`. Non-fresh classes sight by content digest. |
