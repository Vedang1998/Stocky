...
| Q-011 | Phase 1 foundation must add database-enforced tenant isolation (canonical Shop, shopId, composite tenant constraints, forced RLS, restricted runtime role, separate migration role, transaction-local context, bootstrap exception, real PostgreSQL and pool isolation tests) — Claude F-016 / R-022 | **P1 gating requirement.** Application-layer shop filters alone are insufficient. Planning direction approved. PR 1 tooling is merged (D-026) and remains nullable ownership/diagnostics only — **not** enforcement. PR 2 application scoping (including third-cycle corrections under D-030) does **not** close this gate; PR 3 remains the hard database-enforcement gate. | Product + Cursor (Phase 1) | Phase 1 foundation — **mandatory implementation gate until enforcement is merged and independently verified** | **Open — PR 1 merged for expansion/backfill only; PR 2 remain unaccepted under third correction cycle; enforcement not implemented** |
...
**PR 2 review note:** The first PR 2 implementation head `6f9ca22c069a46003b6944ff56c888ff91e95cdc` was independently rejected (`NOT READY — CORRECTIONS REQUIRED`). The first correction wave at `e6a9a06a8a399bbfb17687399c59582f1712f442` was independently rejected (`NOT READY — FURTHER CORRECTIONS REQUIRED`; report commit `b5fbd2b…`). The second correction wave at `99d7a2bb73e77f62bd4ed0029961b40ab04a08e0` was independently rejected (`NOT READY — FURTHER CORRECTIONS REQUIRED`; report commit `fed21a48…`; no cross-tenant read/write reproduced; three functional/availability P1 findings). Third-cycle corrections proceed under D-030 on draft PR #13. PR 3 database enforcement remains a later hard gate and is not started.