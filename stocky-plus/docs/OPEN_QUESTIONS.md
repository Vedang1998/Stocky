...
| Q-011 | Phase 1 foundation must add database-enforced tenant isolation (canonical Shop, shopId, composite tenant constraints, forced RLS, restricted runtime role, separate migration role, transaction-local context, bootstrap exception, real PostgreSQL and pool isolation tests) — Claude F-016 / R-022 | **P1 gating requirement.** Application-layer shop filters alone are insufficient. Planning direction approved. PR 1 tooling is merged (D-026) and remains nullable ownership/diagnostics only — **not** enforcement. | Product + Cursor (Phase 1) | Phase 1 foundation — **mandatory implementation gate until enforcement is merged and independently verified** | **Open — PR 1 merged for expansion/backfill only; enforcement not implemented** |
