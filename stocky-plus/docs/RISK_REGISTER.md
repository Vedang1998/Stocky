...
| R-015 | P1 | `main` may merge without required CI | PR #6 merged while Actions run `30470541851` was red | **API-VERIFIED:** ruleset `Protect main` (id `20012314`, enforcement `active`, target `~DEFAULT_BRANCH`) requires PRs; required check `Lint, typecheck, test, build, Prisma, GraphQL` with `strict_required_status_checks_policy`; squash-only merges; `non_fast_forward` blocks force pushes; deletion restricted; bypass actors empty; draft PRs cannot merge until marked ready under GitHub draft workflow | Future settings drift could weaken protection | Periodically re-verify ruleset remains active |
