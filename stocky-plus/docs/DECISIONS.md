...
9. **Risks:** Record PR 5 planning risks R-129 through R-158. Do not close R-010, R-014, R-028, R-029, R-034, R-095..R-098, or Q-002/Q-004 by planning approval. Preserve R-122/R-123 as accepted PR 4 residuals. Preserve R-102 / R-137. Do not close implementation risks.
...
12. **Planning correction 2 (same D-053 — do not create D-054):** ChatGPT required documentation-only corrections for clock-domain split (Shopify attribute version vs authoritative existence observation vs webhook signal observation), delete/disconnect as signal plus authoritative existence check, un-tombstone without comparing Shopify `updatedAt` to local tombstone time, full-sync presence marker independent of stale-attribute no-op, absence-tombstone by existence evidence, and nullable Shopify version fallback. These remain planning rules only. Implementation is still **NOT AUTHORIZED**. No D-054.
13. **Planning correction 3 (same D-053 — do not create D-054):** After the immutable Claude independent review of planning head `b33cf33…` (report commit `541c652…`, blob `f6e62fe…`), ChatGPT required documentation-only architecture corrections: replace `Shop.catalogObservationGen` with platform sequence `stocky_catalog_observation_gen_seq`; allocate observation generations immediately after Shopify response completion and before identity-lock wait; treat COMPLETED bulk omission as absence **candidates** plus bounded confirmation and a configurable blast-radius circuit breaker (not `ABSENT_FULL_SYNC_SWEEP`); canonical InventoryLevel identity `(shopId, inventoryItemGid, locationGid)`; two-phase checkpoint only; merchant-durable diagnostics with a derived `DataIssue`/`SyncHealth` reconciler; READ COMMITTED candidate sweeps; bounded terminal-GID revival; deny-by-default write scanner (R-110); unitCost non-bulk capability preflight; and P3 payload/media/`bulkOperations`/currency-provenance fixes. These remain planning rules only. Implementation is still **NOT AUTHORIZED**. No D-054.
14. **Planning correction 4 (same D-053 — do not create D-054):** ChatGPT required a documentation-only correction before Claude Code re-review: sequence privileges are **USAGE only** for `stocky_runtime` and `stocky_control_plane` (no SELECT, no UPDATE, no ownership, no PUBLIC, no schema-wide sequence grant, explicit NO CYCLE, `setval` denied); official PostgreSQL 18 `nextval` requires USAGE or UPDATE and `setval` requires UPDATE, so UPDATE is forbidden because it would permit generation reset/reuse. Direct authoritative Shopify queries use an app-issued observation interval `[observationRequestGen, observationResponseGen]` (start generation **before** the network request; end generation **after** a usable response; then identity lock; no row lock across Shopify I/O). Generations order **app request lifecycle only** and do **not** prove Shopify mutation/snapshot order. Overlapping conflicting existence observations preserve last unambiguous state, persist `CONCURRENT_EXISTENCE_OBSERVATION_CONFLICT`, and refetch after both complete; agreeing overlaps may converge. Null-version attribute conflicts follow the same overlap rule. Full-sync remains fence-before-submit, candidates-only, with no tombstone while an overlapping LIVE direct check is unresolved. Open risks R-157 and R-158. Tests AE–AL are mandatory in addition to A–AD. These remain planning rules only. Implementation is still **NOT AUTHORIZED**. No D-054.
15. **Planning correction 5 (same D-053 — do not create D-054):** After the immutable Claude correction-4 independent review of planning head `f03148e…` (report commit `f7b78f1…`, blob `e645c81…`), ChatGPT required a documentation-only architecture correction for **F-CLAUDE-PR5C4-01** (P2) and **F-CLAUDE-PR5C4-02** (P3). Concurrent direct observations use a merchant-domain `CatalogObservationInFlight` (or equivalent) that supports **multiple simultaneous** rows per canonical identity — not one mutable current-in-flight slot. `observationRequestGen` is not merchant identity. No cross-role FK to PR 4 control-plane tables. Every ACTIVE unresolved observation has a **finite** `leaseExpiresAt` derived from the configured finite Shopify request timeout plus a finite bounded recovery margin, with a validated finite maximum and test-configurable short values. Wall-clock lease time is liveness/recovery only and does not order Shopify mutations, existence, or attributes. An ACTIVE, UNEXPIRED, resultless observation participates as `[requestGen, +∞)` and blocks overlapping later observations from **mutating canonical existence**, but does **not** prevent the later Shopify request from being issued. A held later response is not replayed as fresh; bounded PR 4 retry/refetch obtains a new observation after the blocker settles. Lease expiry is **logical abandonment** even if the row is not yet rewritten: the expired resultless observation does not block, creates no LIVE/ABSENT fact, never authorizes deletion or revival, and burns its request generation. Physical cleanup is maintenance, not the correctness boundary. The tenant fact transaction **fences** the observation token (expected, ACTIVE, not abandoned, not expired); a late worker cannot apply, tombstone, revive, update null-version attributes, or clear newer evidence after expiry/abandonment. Do **not** grant `stocky_control_plane` DML on merchant fact/in-flight tables; no impossible cross-role atomic transaction. Terminal-GID revival’s two LIVE confirmations **must be non-overlapping** (`second.observationRequestGen > first.observationResponseGen`). Open risk **R-159**. Mandatory tests **AM** and **AN**; Race **AB** extended. R-157 and R-158 remain OPEN. R-129 through R-158 remain OPEN. These remain planning rules only. Implementation is still **NOT AUTHORIZED**. No D-054.
16. **Planning correction 6 (same D-053 — do not create D-054):** After the immutable Claude correction-5 independent review of planning head `8262bdb…` (report commit `1c1cba2…`, blob `c465b7d…`), ChatGPT required a documentation-only specification of the Correction-5 lease without redesigning that architecture, closing **F-CLAUDE-PR5C5-01** (P2), **F-CLAUDE-PR5C5-02** (P3), **F-CLAUDE-PR5C5-03** (P3), and **F-CLAUDE-PR5C5-04** (P3). PostgreSQL database time is the **sole** authoritative clock for observation-lease creation and expiry. The authoritative primitive is `clock_timestamp()` (official PostgreSQL 18: `CURRENT_TIMESTAMP` / `now()` / `transaction_timestamp()` freeze at transaction start; `statement_timestamp()` is statement-start time; `clock_timestamp()` returns actual server time when evaluated and advances during statement execution). The absolute `leaseExpiresAt` is computed in PostgreSQL as `clock_timestamp() + validated_bounded_lease_interval`; the application may supply only the already-validated finite duration and **must not** supply absolute current time from `Date.now()`, `new Date()`, process uptime, worker-local timers, container/VM clock, response-arrival wall time, or another worker’s timestamp. Those clocks are logs/observability only. Valid iff `clock_timestamp() < leaseExpiresAt`; expired iff `clock_timestamp() >= leaseExpiresAt` (exact equality is expired). The same operator applies to every PR5 lease predicate. The critical fence is at **fact application time** after canonical-identity / observation locking: re-evaluate `clock_timestamp() < leaseExpiresAt`; do not cache `dbNow` across a lock wait; do not use transaction-start or statement-start time as the post-lock fence. Zero matching in-flight rows fail closed; more than one row for the same exact token also fails closed; a stale worker must not recreate a missing row to apply. Blocker evaluation is existential across **all** overlapping ACTIVE unexpired resultless observations. Lease time remains liveness/fencing only, not Shopify ordering. Extend R-159 (still **OPEN** P2). Preserve races A–AN; extend Race AM; add Races AO / AP / AQ / AR. R-157 and R-158 remain OPEN. R-129 through R-159 remain OPEN. These remain planning rules only. Implementation is still **NOT AUTHORIZED**. No D-054.
17. **Planning correction 7 (same D-053 — do not create D-054):** After Correction 6 established PostgreSQL `clock_timestamp()` as the sole lease clock at planning head `6dd041d…`, ChatGPT required a documentation-only correction to close the remaining stale-token revalidation path **without** redesigning Correction 5/6. Official PostgreSQL 18 documents `clock_timestamp()` as actual server wall time when evaluated; it is **not** a documented monotonic business clock. Distinguish **lease invalidity** (`clock_timestamp() >= leaseExpiresAt` at the original observation’s final fact fence — original cannot apply) from **durable abandonment** used to permit a successor mutation. If another observation wants to stop treating an expired ACTIVE resultless observation as a blocker and proceed with canonical mutation, it **MUST** first durably fence that exact row `ACTIVE -> ABANDONED` inside the same tenant-scoped canonical-identity transaction, conditional on exact `shopId`, exact canonical identity, exact observation token, `lifecycleState = ACTIVE`, resultless, and `clock_timestamp() >= leaseExpiresAt`. `ABANDONED` never returns to `ACTIVE`. Physical `DELETE`/reaping remains maintenance; the `ACTIVE -> ABANDONED` transition when expiry is relied upon is part of the correctness/fencing boundary. Successor correctness must not depend on a background pass. Under the identity lock: fence all expired overlapping ACTIVE resultless blockers, re-evaluate, remain blocked if any ACTIVE unexpired resultless row survives, then continue only through existing interval/conflict/fact rules; successor mutation and abandonment fencing are the same tenant transaction — rollback undoes both. No cross-role transaction. CASE 1 backward clock before any successor relies on expiry is a liveness delay. CASE 2 after durable `ABANDONED`: clock rollback cannot reactivate. Original worker still requires exactly one matching token row that is ACTIVE, not ABANDONED, and `clock_timestamp() < leaseExpiresAt`. Extend Race AQ. Add Race AS. Extend R-159 (still **OPEN** P2). R-157 and R-158 remain OPEN. R-129 through R-159 remain OPEN. These remain planning rules only. Implementation is still **NOT AUTHORIZED**. No D-054.
18. **Planning correction 8 (same D-053 — do not create D-054):** After the immutable Claude final Corrections 6+7 independent review of planning head `2db55b6…` (report commit `a757baa…`, blob `b1c4265…`), ChatGPT required a documentation-only correction to close **F-CLAUDE-PR5C7-01** (P1), **F-CLAUDE-PR5C7-02** (P2), **F-CLAUDE-PR5C7-03** (P3), and **F-CLAUDE-PR5C7-04** (P3) **without** redesigning the accepted observation-interval, lease, clock, deletion, bulk-ingest, or tenant architecture. Official PostgreSQL 18: `SELECT … FOR UPDATE` locks retrieved rows only; advisory locks are session-level (`pg_advisory_lock`, does not honor transaction rollback) or transaction-level (`pg_advisory_xact_lock`, released at transaction end). Every canonical fact-application transaction **MUST** first acquire exclusive transaction-scoped `pg_advisory_xact_lock(key1, key2)` for the exact canonical identity — the PRIMARY serialization boundary whether the fact row exists, does not exist, was tombstoned, is first-created, or is an InventoryLevel pair. Versioned length-prefixed preimage `stocky-pr5-canonical-lock-v1` + `shopId` + `resourceKind` + identity fields; SHA-256; first 8 digest bytes as two signed 32-bit integers; never a 64-bit JavaScript `Number`. Hash collisions may over-serialize only. `SELECT … FOR UPDATE` remains SECONDARY when the row exists. No advisory lock across Shopify I/O. No canonical writer may bypass the anchor. First insert re-reads after the anchor; `INSERT … ON CONFLICT DO UPDATE` is not a substitute for the apply algorithm. Multi-identity batches acquire deduplicated advisory keys in ascending `(key1, key2)` order; observation-row locks follow in ascending `observationRequestGen` / token order. `observationResponseGen` is allocated after a usable response (Correction 4) but kept in-process and persisted only atomically when the observation leaves `ACTIVE`; committed `ACTIVE` + non-null `responseGen` is forbidden; planned DB invariant (`ACTIVE` ⇒ NULL; `COMPLETED` ⇒ NOT NULL; `ABANDONED` NULL or non-null only when a usable response is discarded atomically). Every durably ACTIVE row is resultless, so Correction 7 remains coherent. Add Race AT (first insert), Race AU (no response-bearing ACTIVE), Race AV (deterministic lock order). Update §16 D1.14 to P–AV. Open **R-160** (P1; do not close from planning). Extend R-159 (still **OPEN** P2). R-157 and R-158 remain OPEN. R-129 through R-160 remain OPEN. These remain planning rules only. Implementation is still **NOT AUTHORIZED**. No D-054.
19. **Planning acceptance and merge identity (same D-053 — not implementation authorization):** Independent Claude correction-8 review of planning head `a7d23bcfb6d82a209a986302e10b3a9a46601c87` issued verdict **`APPROVE PR5 PLANNING`** with residual findings **F-CLAUDE-PR5C8-01** (P2) and **F-CLAUDE-PR5C8-02** (P3). Final review-record commit before squash: `1691933ec126eed44de81162e8492fb7f0bfae0c`. Immutable review blob: `0d322db701f5f27b89bc4069e6fb1f3d751d15a3` (`PR5_PLANNING_CORRECTION_8_INDEPENDENT_REVIEW.md` — never edit). After ChatGPT planning acceptance and explicit user squash-merge authorization, PR [#24](https://github.com/Vedang1998/Stocky/pull/24) is **CLOSED and MERGED**. Squash merge / current `origin/main`: `edabd8de1f1b25cc5f5f1026e34ddf69aa104f7e` at `2026-08-16T16:49:46Z`. Post-merge main CI run `31959761072`, event `push`, head `edabd8de…`, completed **success** (Classify job `95195836526` SUCCESS; CI Gate job `95195850559` SUCCESS; Heavy job `95195850790` SKIPPED). Planning is **ACCEPTED AND MERGED**. Implementation remains **NOT STARTED** and **NOT AUTHORIZED** until later **D-054** activation conditions complete. Do **not** edit any immutable PR 5 review artifact.
20. **Residual planning findings deferred to implementation-entry:** F-CLAUDE-PR5C8-01 and F-CLAUDE-PR5C8-02 must be resolved in the documentation-only implementation-entry contract **before** runtime implementation. They do **not** reopen D-053 planning. They do **not** by themselves make D-054 effective.

## D-054 — Phase 1 PR 5 implementation authorization under Accelerated Safe Delivery v1

1. **Current:** Phase 1 PR 4 remains **FORMALLY CLOSED** under **D-052**. Phase 1 PR 5 planning under **D-053** is **ACCEPTED AND MERGED**. PR [#24](https://github.com/Vedang1998/Stocky/pull/24) is **CLOSED / MERGED**. Squash merge / `origin/main`: `edabd8de1f1b25cc5f5f1026e34ddf69aa104f7e`. Planning review head before squash: `1691933ec126eed44de81162e8492fb7f0bfae0c`. Final immutable review blob: `0d322db701f5f27b89bc4069e6fb1f3d751d15a3`. Independent verdict: **`APPROVE PR5 PLANNING`**. Post-merge main CI run `31959761072` **SUCCESS**. Residual findings **F-CLAUDE-PR5C8-01** (P2) and **F-CLAUDE-PR5C8-02** (P3) are open pre-implementation obligations. The implementation branch `phase-1/catalog-location-inventory-facts` is **absent**.
2. **Proposed / decided:** Record **conditional** Phase 1 PR 5 implementation authorization under Accelerated Safe Delivery v1, and resolve F-CLAUDE-PR5C8-01 / F-CLAUDE-PR5C8-02 in the implementation-entry contract **without** authorizing runtime work in this documentation PR.
3. **Exact wording:** **Phase 1 PR 5 implementation authorization under Accelerated Safe Delivery v1.**
4. **Conditional status:** D-054 is **CONDITIONAL** while the implementation-entry PR is open. It is **NOT EFFECTIVE** merely because the heading exists. Do **not** mark D-054 effective in this PR.
5. **Activation:** D-054 becomes **EFFECTIVE** only if **ALL** are true:
...
6. **After condition 9 only:** `phase-1/catalog-location-inventory-facts` may be created and PR 5 runtime implementation may begin.
7. **F-CLAUDE-PR5C8-01 resolution (planning contract only):** Keep `pg_advisory_xact_lock(key1, key2)` as the transaction-scoped canonical identity anchor. Separate JSONL read batch from canonical-apply transaction batch. Initial identities-per-canonical-fact-transaction default **32** (engineering hypothesis; configurable downward; increase requires capacity evidence). Verify `max_locks_per_transaction`, `max_connections`, and `max_prepared_transactions`. Planning estimate `shared_lock_object_budget = max_locks_per_transaction * (max_connections + max_prepared_transactions)` is **not** a guarantee that every slot is available to PR 5. Accept a configured multi-identity batch only when both `canonicalLocksPerTransaction <= floor(max_locks_per_transaction / 2)` and `canonicalLocksPerTransaction * configuredWorstCaseConcurrentCanonicalTransactions <= floor(shared_lock_object_budget * 0.25)`. Automatically reduce the effective batch cap when the initial 32 is unsafe. Never increase PostgreSQL settings automatically. Never alter `max_locks_per_transaction` at runtime. Never require a database restart from application code. Worst-case concurrent canonical multi-identity fact transactions default **4**. Fail closed on lock-table exhaustion: abort; commit no canonical changes; commit no half-applied abandonment fencing; release transaction advisory locks through rollback; bounded backoff; reduce sub-batch; never split one identity; never unanchored fallback; stop at one identity; then degrade / fail the job. Mandatory Race **AW**. Open **R-161**. Do **not** close R-161 in this docs PR.
8. **F-CLAUDE-PR5C8-02 resolution (planning contract only):** Pin encoding version `stocky-pr5-canonical-lock-v1`. Each UTF-8 component is `<decimal UTF-8 byte length>:<UTF-8 bytes>` with no extra separator. Byte length is authoritative. No trimming, lowercasing, Unicode normalization, or Shopify-domain normalization. `shopId` is the exact stored Prisma `Shop.id` cuid string bytes. Resource kinds are exact literals `Product`, `ProductVariant`, `InventoryItem`, `Location`, `InventoryLevel`. SHA-256 digest bytes 0..7 become signed 32-bit big-endian `key1` / `key2`. Never convert the first eight bytes into a JavaScript `Number`. One canonical key-derivation function. Known-answer vectors 1–3 are pinned in the brief and independently reproduced. Extend **R-160** (still **OPEN**).
9. **Accelerated Safe Delivery v1:** Canonical document `stocky-plus/docs/ACCELERATED_SAFE_DELIVERY.md` is permanent repository governance for ChatGPT, Cursor, and Claude Code. Safety gates do not change. Parallelize work, not uncertainty. One dependency level ahead of planning is permitted when expressly authorized and is not implementation authorization. Parallel Cursor lanes exist only when ChatGPT defines them. Foundation first. Small PRs. Risk tiers do not override phase gates. Claude early red-team plus final exact-head review. Consolidated corrections. Durable GitHub handoffs. No CI workflow change in this docs PR.
10. **Explicit non-authorization:** D-054 does **not** authorize production, merchant production data, enabling inventory-write flags, Shopify inventory mutations, Phase 2 runtime, or PR 6 runtime before its own authority. This documentation PR does **not** create the implementation branch and does **not** change runtime, schema, migration, tests, packages, Shopify configuration, GraphQL, or `.github` workflows.
11. **Merchant impact:** None in production. No production or merchant-data change is authorized.
12. **Technical impact:** Documentation only. Implementation remains **NOT STARTED / NOT AUTHORIZED YET**.
13. **Migration:** None authorized.
14. **Risks:** R-157, R-158, R-159, and R-160 remain **OPEN** until implementation/review. R-161 is **OPEN — PR 5 implementation entry**. Do not close R-010, R-014, R-028, R-029, R-034, R-095..R-098, or Q-002/Q-004. Do not create D-055.
15. **Final (implementation-entry PR, historical):** **D-054 WAS CONDITIONAL / NOT EFFECTIVE** while PR #26 was open. That heading alone was not authority.
16. **Activation complete (same D-054 — do not create D-055):** All nine activation conditions are now true. PR [#26](https://github.com/Vedang1998/Stocky/pull/26) is **CLOSED / MERGED**. Accepted review-record head: `7171c2cbbdff15ae0d92aa6850a1ae12804db3f4`. Squash merge / `origin/main`: `ae1b428039152efc6b4a46107e1bcca5eb17586a`. Post-merge main CI run `31966584542`, event `push`, head `ae1b4280…`, **SUCCESS** (Classify `95212558793` SUCCESS; CI Gate `95212578956` SUCCESS; Heavy `95212579347` SKIPPED because PR26 was docs-only). D-054 is therefore **EFFECTIVE**. PR 5 runtime implementation is authorized. The implementation branch `phase-1/catalog-location-inventory-facts` is in use for **PR5-F1 FOUNDATION IN PROGRESS**. R-157, R-158, R-159, R-160, and R-161 remain **OPEN**. Production remains unauthorized. Inventory-write flags remain **DEFAULT OFF**. Do **not** create D-055. Do **not** state PR 5 is complete. Do **not** close Phase 1.
17. **PR5-F1 foundation acceptance / merge identity (same D-054 — do not create D-055):** After independent Claude correction re-review of accepted final review-record head `56c764d00f8350cf22e8b37acf5c61a5b5757e7b` issued verdict **`APPROVE PR5-F1 FOUNDATION CORRECTION`** (P0=0 P1=0 P2=0 P3=4), ChatGPT technically accepted the PR5-F1 foundation. The user explicitly authorized squash merge of PR [#27](https://github.com/Vedang1998/Stocky/pull/27). PR #27 is **CLOSED / MERGED**. Squash merge / `origin/main` at F1 merge: `7827e535415c9acbacfbbb4bdedff08be6650d5c` at `2026-08-17T13:48:17Z`. Pre-merge final exact-head PR CI run `31988065401`, event `pull_request`, head `56c764d0…`, **SUCCESS**. Post-merge main CI run `32036740386`, event `push`, head `7827e535…`, **SUCCESS** (Classify `95408642308` SUCCESS; Full Heavy `95408670595` SUCCESS; CI Gate `95417341718` SUCCESS). The PR5-F1 foundation is **ACCEPTED / MERGED / FROZEN**. Shared schema / tenant / identity / transaction contracts may now be consumed by separately authorized downstream PR5 lanes. Parallel lanes may begin only when ChatGPT separately defines them under Accelerated Safe Delivery v1. D-054 remains the implementation authority. R-157, R-158, R-159, R-160, and R-161 remain **OPEN**. Production remains unauthorized. Inventory-write flags remain **DEFAULT OFF**. Do **not** create D-055. Do **not** state PR 5 is complete. Do **not** close Phase 1.
18. **PR5-F2A admin-read acceptance / merge identity (same D-054 — do not create D-055):** After independent Claude S01 re-review of accepted implementation head `bfbe369f590e38f36de8165e366dd7e84449ecd7` issued verdict **`APPROVE PR5-F2A ADMIN READ S01 CORRECTION`** (P0=0 P1=0 P2=0), ChatGPT technically accepted the PR5-F2A admin-read boundary. The user explicitly authorized squash merge of PR [#29](https://github.com/Vedang1998/Stocky/pull/29). PR #29 is **CLOSED / MERGED**. Previous main: `5129707ee684e66cadcf96b976e16eb57385a7cb`. Squash merge: `f65ab4b906f53b3a1c72cdd7b29cdc0cbde6a7d7` at `2026-08-20T11:04:26Z`. Implementation-head exact-head PR CI run `32263496048`, event `pull_request`, head `bfbe369f…`, **SUCCESS**. Post-merge main CI run `32362021387`, event `push`, head `f65ab4b…`, **SUCCESS** (Classify `96403425899` SUCCESS; Full Heavy `96403462492` SUCCESS; CI Gate `96415720267` SUCCESS). Immutable review blobs (never edit): first `81bc0678ea9041b6567c02c8fe5655752fc53441`; correction `d06fc9f603b8ec86efc1493babaa3973a73d3806`; second correction `acbd51277319d0737861355d1db5b5068a070747`; S01 `bba424c0dd8f3903bdffe79ffe803269b2dd2fd9`. Closure report: `phases/phase-1/PR5_F2A_ADMIN_READ_CLOSURE_REPORT.md`. The F2A admin-read sub-lane satisfied **its own** R-163 scanner obligation on `app/lib/catalog-facts`. **R-163 remains globally OPEN** until F3 proves both scanner roots (`app/lib/catalog-facts/**` and `app/jobs/workers/catalog-facts/**`). Do **not** convert the F2A-lane satisfaction into global closure. R-016 / R-132 / R-134 / R-136 / R-138 remain **OPEN**. Later downstream after this F2A identity: F2B (#31) **MERGED**; F2C (#30) **MERGED**; PR #32 remaining-integration planning **MERGED** as `f1201f853b8a42f40e4d3e5565b6406410360c8a`; F3 runtime **NOT STARTED**; PR6 runtime **NOT AUTHORIZED**. D-054 remains the implementation authority. Production remains unauthorized. Inventory-write flags remain **DEFAULT OFF**. Do **not** create D-055. Do **not** state PR 5 is complete. Do **not** close Phase 1.
19. **Emergency Continuity Sprint control packet (same D-054 / Accelerated Safe Delivery v1 — do not create D-055):** Stocky became unavailable after 31 August 2026. ChatGPT records an **INTERNAL / CONTROLLED** operational rescue. As of the 2026-09-05 current-state addendum, Monday 7 September 2026 remains the **upcoming INTERNAL / CONTROLLED operational target** originally recorded on 2026-09-01. It is not a completion claim and does not authorize F3 runtime. This is **not** full public App Store parity by that date. The approved 132-feature product scope is unchanged. Safety gates are **not** relaxed. Delivery uses Accelerated Safe Delivery v1 with a **maximum of four** independent Cursor lanes, one writer per branch, Claude as independent Tier-A reviewer, and mandatory exact-head CI. Inventory-write flags remain **DEFAULT OFF**. `FEATURE_PR5_ABSENCE_TOMBSTONE` remains **DEFAULT OFF**. Shopify remains authoritative. Shopify-native purchase orders / receiving / transfers / adjustments are the emergency write bridge until equivalent app writes pass later safety gates. Monday rescue priorities remain reliable inventory facts, replenishment, ABC/U, low-stock, editable quantities, and a Shopify-compatible ordering/export flow. Full commercial launch remains subject to normal phase/release gates. Durable record: `EMERGENCY_DELIVERY_DIRECTIVE_2026-09-01.md`. This item is operational calendar control under existing D-054 / Accelerated Safe Delivery. It is **not** a new product-rule decision number. It does **not** start F3 runtime. After this packet was first recorded, F2B, F2C, and PR #32 planning merged on `main`; those later merges do not convert this packet into F3 authorization. Do **not** create D-055.
20. **PR5-F3 remaining-integration runtime authorization (same D-054 — do not create D-055):** On **2026-09-05**, after PR [#33](https://github.com/Vedang1998/Stocky/pull/33) squash-merged as exact `main` `28c810090394f319e599fc6c501b898befa39cad` and post-merge run `33978361886` succeeded on that exact SHA, ChatGPT expressly authorized the single PR5-F3 remaining-integration runtime lane under existing **D-054 EFFECTIVE**. The implementation boundary is the accepted one-PR integration of JSONL bulk ingestion, paired checkpoint/resume, authoritative webhook refetch, absence nomination and confirmation/reconcile, compatibility-projection integration, v1 legacy-authority fencing, the recursive two-root safety scanner, and catalog/inventory health. This authorization is **not D-055** and does not alter approved product rules. Production, merchant production data, deployment, Shopify inventory mutations, and every inventory-write flag remain **NOT AUTHORIZED / DEFAULT OFF**. `FEATURE_PR5_ABSENCE_TOMBSTONE` must be implemented **DEFAULT OFF** and is not authorized for enablement. PR6 runtime remains **NOT AUTHORIZED**. PR #34 remains untouched. The F3 pull request must remain DRAFT and UNMERGED pending exact-head full CI, independent Claude review requested separately by ChatGPT, ChatGPT technical acceptance, and explicit user merge authorization.
