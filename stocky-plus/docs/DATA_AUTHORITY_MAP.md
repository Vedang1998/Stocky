...
| Orders / refunds | Shopify | Legacy webhooks enqueue → `SalesDailyAggregate` only (not PR6 source of record). PR6-A schema/security and PR6-B Admin READ plus PR6-C canonical applicator are **merged** at squash **V** (`a3ff480f…`). Intended unit-event ledger is `ShopifyOrderAgreementSaleFact`. **PR6-D complete webhook/import/reconcile integration is implemented on draft PR [#43](https://github.com/Vedang1998/Stocky/pull/43)** (not merged, not production, not live store calls, not live subscription registration). | Production order/line/refund fact runtime remains unauthorized until independent Claude review, ChatGPT acceptance, and explicit merge. Refunds remain incomplete for production audits. |
