/**
 * Query helpers for the order applicator. No Shopify I/O.
 *
 ...
 * argument list.
 */
import type { OrderLockQueryRaw } from "../advisory-lock";
import { OrderApplyUniqueConflictError } from "./errors";

export type OrderApplyDb = OrderLockQueryRaw;

export function queryRows<T = Record<string, unknown>>(
  db: OrderApplyDb,
): (strings: TemplateStringsArray, ...values: unknown[]) => Promise<T[]> {
  return async (strings, ...values) => {
    const tag: OrderLockQueryRaw["$queryRaw"] = db.$queryRaw.bind(db);
    const result = await tag(strings, ...values);
    if (!Array.isArray(result)) {
      throw new Error("order apply query must return an array of rows");
    }
    return result as T[];
  };
}
...
export function throwIfUniqueViolation(error: unknown): never {
  if (isUniqueViolation(error)) {
    throw new OrderApplyUniqueConflictError();
  }
  throw error;
}

export function newFactId(prefix = "ofa"): string {
