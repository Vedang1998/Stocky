/**
 * tenant-job-envelope-v3 — binds durable job + dispatch identity + queue job ID.
 * v2 remains parseable for in-flight jobs; new dispatches use v3 only.
 */
...
export const TENANT_JOB_ENVELOPE_V3_VERSION = "tenant-job-envelope-v3" as const;

export type TenantJobEnvelopeV3 = Readonly<{
  schemaVersion: typeof TENANT_JOB_ENVELOPE_V3_VERSION;
  durableJobId: string;
  dispatchId: string;
...
export type TenantJobContextV3 = {
  envelope: TenantJobEnvelopeV3;
  tenant: TenantAuthority;
  db: TenantDb;
};

type UnsignedV3 = Omit<TenantJobEnvelopeV3, "signature">;

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

export function serializeUnsignedEnvelopeV3(unsigned: UnsignedV3): string {
  return JSON.stringify({
    causationId: unsigned.causationId ?? null,
    correlationId: unsigned.correlationId,
    dispatchId: unsigned.dispatchId,
    dispatchSequence: unsigned.dispatchSequence,
    durableJobId: unsigned.durableJobId,
    issuedAt: unsigned.issuedAt,
    myshopifyDomain: unsigned.myshopifyDomain,
    payloadDigest: unsigned.payloadDigest,
    queueJobId: unsigned.queueJobId,
...
export function signTenantJobEnvelopeV3(
  unsigned: UnsignedV3,
  secret: Buffer = requireTenantJobEnvelopeSecret(),
): string {
  return createHmac("sha256", secret)
    .update(serializeUnsignedEnvelopeV3(unsigned))
    .digest("base64url");
}
...
function verifySignatureV3(
  ...
  const expected = signTenantJobEnvelopeV3(unsigned, secret);
  const a = Buffer.from(expected);
  const b = Buffer.from(signature);
  if (a.length !== b.length || !timingSafeEqual(a, b)) {
    throw new TenantAuthorityError(
      "envelope_signature_invalid",
      "Job envelope v3 signature verification failed",
    );
  }
}
...
export function createTenantJobEnvelopeV3(input: {
  tenant: TenantAuthority;
  source: TenantJobSource;
  durableJobId: string;
  dispatchId: string;
  ...
  if (!input.durableJobId) {
    throw new TenantAuthorityError(
      ...
      "Envelope v3 requires durableJobId",
    );
  }
  if (!input.dispatchId) {
    throw new TenantAuthorityError(
      "missing_envelope_dispatch_id",
      "Envelope v3 requires dispatchId",
    );
  }
  if (
    !Number.isInteger(input.dispatchSequence) ||
    input.dispatchSequence < 1
  ) {
    throw new TenantAuthorityError(
      "missing_envelope_dispatch_sequence",
      "Envelope v3 requires a positive integer dispatchSequence",
    );
  }
  if (!input.queueJobId) {
    throw new TenantAuthorityError(
      "missing_envelope_queue_job_id",
      "Envelope v3 requires queueJobId",
    );
  }
  if (!input.payloadDigest || input.payloadDigest.length !== 64) {
    throw new TenantAuthorityError(
      "missing_envelope_payload_digest",
      "Envelope v3 requires a 64-char hex payloadDigest",
    );
  }

  const secret = requireTenantJobEnvelopeSecret();
  const unsigned: UnsignedV3 = {
    schemaVersion: TENANT_JOB_ENVELOPE_V3_VERSION,
  ...
  return {
    ...unsigned,
    signature: signTenantJobEnvelopeV3(unsigned, secret),
  };
}

export function parseTenantJobEnvelopeV3(
  raw: unknown,
  ...
  if (raw.schemaVersion !== TENANT_JOB_ENVELOPE_V3_VERSION) {
    throw new TenantAuthorityError(
  ...
  if (typeof raw.dispatchId !== "string" || raw.dispatchId.length === 0) {
    throw new TenantAuthorityError(
      "missing_envelope_dispatch_id",
      "Job envelope missing dispatchId",
    );
  }
  if (
    typeof raw.dispatchSequence !== "number" ||
    !Number.isInteger(raw.dispatchSequence) ||
    raw.dispatchSequence < 1
  ) {
    throw new TenantAuthorityError(
      "missing_envelope_dispatch_sequence",
      "Job envelope missing valid dispatchSequence",
    );
  }
  if (typeof raw.queueJobId !== "string" || raw.queueJobId.length === 0) {
    throw new TenantAuthorityError(
      "missing_envelope_queue_job_id",
      "Job envelope missing queueJobId",
    );
  }
  ...
  const unsigned: UnsignedV3 = {
    ...
    dispatchSequence: raw.dispatchSequence,
  ...
  verifySignatureV3(unsigned, raw.signature, secret);

  return { ...unsigned, signature: raw.signature };
}

export async function resolveTenantJobContextV3(
  rawEnvelope: unknown,
  options?: {
    ...
    expectedQueueJobId?: string;
    expectedDispatchId?: string;
    expectedDispatchSequence?: number;
    nowMs?: number;
  },
): Promise<TenantJobContextV3> {
  const { assertSourceMatchesJob } = await import("../tenant/job-envelope.server");
  const envelope = parseTenantJobEnvelopeV3(rawEnvelope, {
    nowMs: options?.nowMs,
  });
  ...
  if (
    options?.expectedQueueJobId &&
    options.expectedQueueJobId !== envelope.queueJobId
  ) {
    throw new TenantAuthorityError(
      "envelope_queue_job_mismatch",
      "Envelope queueJobId does not match BullMQ job ID",
    );
  }

  if (
    options?.expectedDispatchId &&
    options.expectedDispatchId !== envelope.dispatchId
  ) {
    throw new TenantAuthorityError(
      "envelope_dispatch_mismatch",
      "Envelope dispatchId does not match JobDispatch",
    );
  }

  if (
    options?.expectedDispatchSequence != null &&
    options.expectedDispatchSequence !== envelope.dispatchSequence
  ) {
    throw new TenantAuthorityError(
      "envelope_dispatch_sequence_mismatch",
      "Envelope dispatchSequence does not match JobDispatch",
    );
  }
